...
define config.name = _("NUGGET THE CORGI PREPARES A HALLOWEEN PARTY")


## Determines if the title given above is shown on the main menu screen. Set this to False to hide the title.
...
## A short name for the game used for executables and directories in the built distribution. This must be ASCII-only, and must not contain spaces, colons, or semicolons.

define build.name = "Nugget the Corgi prepares a Halloween party"


## Sounds and music ############################################################

## These three variables control, among other things, which mixers are shown to the player by default. Setting one of these to False will hide the appropriate mixer.
...
## To allow the user to play a test sound on the sound or voice channel, uncomment a line below and use it to set a sample sound to play.
...
## Transitions #################################################################
##
## These variables set transitions that are used when certain events occur. Each variable should be set to a transition, or None to indicate that no transition should be used.
...
## A variable to set the transition used when the game starts does not exist. Instead, use a with statement after showing the initial scene.
...
## Controls the default text speed. The default, 0, is infinite, while any other number is the number of characters per second to type out.
...
## Save directory ##############################################################
##
## Controls the platform-specific place Ren'Py will place the save files for this game. The save files will be placed in:
##
...
##
## This generally should not be changed, and if it is, should always be a literal string, not an expression.

define config.save_directory = "Nugget the Corgi prepares a Halloween party"
...
init python:
    ...
    ##
    ## For example, "*.txt" matches txt files in the base directory, "game/**.ogg" matches ogg files in the game directory or any of its subdirectories, and "**.psd" matches psd files anywhere in the project.
...
## A Google Play license key is required to download expansion files and perform in-app purchases. It can be found on the "Services & APIs" page of the Google
